  ...
  return (
    <div className="res-page">
      <div className="res-topbar">
        ...
        />
        <Link to="/" className="res-logout-btn">
          Sair
        </Link>
      </div>
      ...
      <div className="res-container">
        ...
        <p className="res-resumo">
          ...
          questões&nbsp; (<strong>{percentual}%</strong> de aproveitamento).
        </p>

        {questoes.map((q, i) => {
          const idxCor = parseInt(q.resposta_correta);
          const idxMar =
            respostas[i] !== undefined && respostas[i] !== null
              ? parseInt(respostas[i])
              : null;
          const acertou = idxMar !== null && idxMar === idxCor;
          const alts = q.alternativas || [];
          const justAlt = Array.isArray(q.justificativas_alternativas)
            ? q.justificativas_alternativas
            : [];
              ...
              <div className="res-q-body">
                ...
                {!acertou && q.justificativa_correta && (
                  <div className="res-just-geral">
                    <span className="chip chip--correta">
                      Por que a resposta correta
                    </span>
                    <div style={{ marginTop: 6 }}>
                      {q.justificativa_correta}
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="res-rodape">
          ...
          {id && (
            <Link to={`/revisao/${id}`} className="res-btn">
              Revisar com IA
            </Link>
          )}
