...
export default function ProfessorPanel() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(({ data }) => setVinculos(data.vinculos));
  }, []);
      ...
      <div className="prof-container">
        ...
        <section className="prof-actions">
          <Link to="/professor/disciplinas/nova" className="prof-card">
            ...
            <span className="desc">
              Crie uma nova disciplina para seus alunos.
            </span>
          </Link>
          <Link to="/professor/disciplinas" className="prof-card">
            ...
            <span className="desc">
              Gerencie, associe alunos e edite detalhes.
            </span>
          </Link>
        </section>
        ...
        >
          <div className="modal">
            ...
            <div className="prof-list">
              {alunos.length === 0 ? (
                <div className="prof-list-row">
                  <span style={{ color: "#64748b" }}>Nenhum aluno.</span>
                </div>
              ) : (
                alunos.map((al) => (
                  <div key={al.id} className="prof-list-row">
                    ...
                    <button
                      className="icon-btn"
                      title="Excluir"
                      onClick={() => excluirAluno(al.id, al.nome)}
                    >
                      <svg viewBox="0 0 24 24">
                        <path d="M9 3h6a1 1 0 0 1 .96.73L16.5 5H19a1 1 0 1 1 0 2h-1v11a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V7H5a1 1 0 0 1 0-2h2.5l.54-1.27A1 1 0 0 1 9 3Zm1 6a1 1 0 0 0-1 1v7a1 1 0 1 0 2 0v-7a1 1 0 0 0-1-1Zm4 0a1 1 0 0 0-1 1v7a1 1 0 1 0 2 0v-7a1 1 0 0 0-1-1Z" />
                      </svg>
                    </button>
                  </div>
                ))
          ...
          <div className="modal">
            ...
            {Object.keys(vinculos).length === 0 ? (
              ...
              Object.entries(vinculos).map(([alunoId, info]) => (
                <div
                  key={alunoId}
                  className={`capsule ${openCaps[alunoId] ? "open" : ""}`}
                >
                  <div
                    className="capsule-header"
                    onClick={() => toggleCap(alunoId)}
                  >
                    <div>
                      <span className="c-name">{info.nome}</span>
                      <span className="c-count">
                        — {info.materias.length} disciplina(s)
                      </span>
