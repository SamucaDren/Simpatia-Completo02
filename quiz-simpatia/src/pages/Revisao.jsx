...
export default function Revisao() {
  ...
  useEffect(() => {
    ...
    setTodasQuestoes(dados.questoes.map((q, i) => ({
      ...
      selecionada: dados.respostas[i] !== undefined ? dados.respostas[i] : null,
      acertou: dados.respostas[i] !== null && dados.respostas[i] === q.resposta_correta,
    })));
  }, [dados]);
  ...
  return (
    <div className="res-page">
      ...
      <div className="res-container">
        <h2>Revisão</h2>
        <p className="res-meta">Tema: <strong>{dados.tema}</strong></p>
        <p className="res-resumo">
          Acertos: <strong>{dados.acertos}</strong> / <strong>{dados.total}</strong>
        </p>

        {dados.questoes.map((q, i) => {
          const idxCor  = q.resposta_correta;
          const idxSel  = dados.respostas[i] !== undefined ? dados.respostas[i] : null;
