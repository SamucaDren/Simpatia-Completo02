...
export default function Dashboard() {
  ...
  const [filtroTema, setFiltro]     = useState('all');

  useEffect(() => {
    const raw = JSON.parse(localStorage.getItem('quiz_historico') || '[]');
    const meu = raw.filter(h => h.userId === 'guest');
    setHistorico(meu);
    setTemas([...new Set(meu.map(h => h.tema))]);
  }, []);

  const dados = historico.filter(h =>
    filtroTema === 'all' || h.tema === filtroTema
  );
  ...
  const chartTodos = {
    ...
    datasets: [{ label: 'Aproveitamento (%)', data: ultimos.map(r => r.percentual),
  ...
  dados.forEach(r => {
    const key = r.percentual >= 80 ? 'facil' : r.percentual >= 50 ? 'medio' : 'dificil';
      ...
      <main className="dash-main">
        <section className="dash-hero">
          ...
          <div className="dash-filtro">
            <label htmlFor="filtro">Tema:</label>
            <select id="filtro" value={filtroTema} onChange={e => setFiltro(e.target.value)}>
        ...
        {dados.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center' }}>
            ...
            <Link to="/" className="dash-btn" style={{ marginTop: 16, display: 'inline-block' }}>
              Fazer um Quiz
            </Link>
          </div>
        ) : (
            ...
            <section className="table-section">
              ...
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr>
                      <th>#</th><th>Tema</th><th>Nível</th><th>Data</th>
