...
export default function ChatWidget({
  ...
  async function enviarMensagem(texto, foco = null) {
    ...
    try {
      const payload = todasQuestoes
        ...
        : { pergunta, tipo };
      const { data } = await api.post("/quiz-simpatia/chat", payload);
      setMensagens((prev) => [...prev, { role: "bot", text: data.resposta }]);
    } catch {
      setMensagens((prev) => [
        ...prev,
        { role: "bot", text: "Erro ao conectar com o assistente." },
      ]);
    } finally {
      setCarregando(false);
    }
  ...
  function startVoice() {
    ...
    if (!SpeechRecognition)
      return alert("Reconhecimento de voz não suportado neste navegador.");
      ...
      {aberto && (
        <div className="chat-box">
          <div className="chat-header">
            {titulo}
            <button className="chat-close" onClick={() => setAberto(false)}>
              ✕
            </button>
          </div>
          ...
          <div className="chat-messages">
            {mensagens.length === 0 && todasQuestoes && (
              <div className="msg bot">
                Olá! Posso explicar qualquer questão do seu quiz. Pergunte, por
                exemplo: <em>"Por que errei a questão 3?"</em>
              </div>
            )}
            {mensagens.map((m, i) => (
          ...
          <div className="chat-input-area">
            <button className="chat-mic-btn" onClick={startVoice} title="Voz">
              🎤
            </button>
            <input
              ...
              placeholder={
                todasQuestoes
                  ? "Ex: Por que errei a questão 2?"
                  : "Como posso te ajudar?"
              }
            />
            <button className="chat-send-btn" onClick={() => enviar()}>
              ➤
            </button>
          </div>
        </div>
