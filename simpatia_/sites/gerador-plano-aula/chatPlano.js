...
function collectFormData() {
  ...
  const numReferenciasComp = document.getElementById(
    "num-referencias-comp",
  ).value;
  const semestre = document.getElementById("semestre").value;
  const ano = document.getElementById("ano").value;

  const avaliacaoCheckboxes = document.querySelectorAll(
    'input[name="avaliacao"]:checked',
  );
  const avaliacaoSelecionada = Array.from(avaliacaoCheckboxes)
    .map((cb) => cb.value)
    .join(", ");

  const metodologiaCheckboxes = document.querySelectorAll(
    'input[name="metodologia"]:checked',
  );
  const metodologiasSelecionadas = Array.from(metodologiaCheckboxes)
    .map((cb) => cb.value)
    .join(", ");
...
function generateConfirmationMessage() {
  ...
  if (!formData.nomeDisciplina && !formData.nomeCurso) {
    appendMessage(
      "Por favor, preencha pelo menos o nome da disciplina ou do curso antes de continuar.",
      "bot",
    );
    return;
  }
...
async function sendMessage() {
  ...
  try {
    ...
    appendMessage(
      "Desculpe, houve um erro de conexão com o servidor. Tente novamente.",
      "bot",
    );
  }
}
