...
function rotuloNivel(n) {
  return (
    { fundamental: "Fundamental", medio: "Médio", superior: "Superior" }[n] ||
    "Médio"
  );
}

// POST /api/quiz-simpatia/quiz/
router.post("/", async (req, res) => {
  const { tema, quantidade, nivel: nivelParam } = req.body;
  const qtd = Math.max(1, Math.min(20, parseInt(quantidade) || 5));
  const nivel = ["fundamental", "medio", "superior"].includes(nivelParam)
    ? nivelParam
    : "medio";
  ...
  const regrasMap = {
    fundamental:
      "Nível FUNDAMENTAL: perguntas simples, vocabulário acessível, frases curtas.",
    medio:
      "Nível MÉDIO: dificuldade intermediária, exige raciocínio e interpretação.",
    superior:
      "Nível SUPERIOR (graduação): questões elaboradas com raciocínio analítico e contextualização.",
  };
...
Formato de cada item:
{
  ...
  try {
    ...
    questoes = questoes.slice(0, qtd).map((q, i) => {
      ...
      if (corretaTexto) {
        const found = alts.findIndex(
          (a) => a.trim().toLowerCase() === corretaTexto.toLowerCase(),
        );
