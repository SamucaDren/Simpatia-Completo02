...
const WATERMARK_TEXT =
  "Documento gerado por IA — sujeito à revisão institucional, sem valor legal.";
...
router.post("/", async (req, res) => {
  ...
  const numSemanas =
    parseInt(context?.semanas) > 0 ? parseInt(context?.semanas) : null;
  ...
  const prompt = `
       ...
       2.  **Cronograma (campo 'desenvolvimento'):** ${
         numSemanas
