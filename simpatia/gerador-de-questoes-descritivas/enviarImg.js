document.addEventListener("DOMContentLoaded", function () {
  ...
  if (uploadImg && questaoImg) {
    ...
    console.error(
      "enviarImg.js: ERRO - Elemento .uploadImg ou .questaoImg não encontrado!",
    ); // ERRO A
  }
  ...
  if (questaoImg) {
    questaoImg.addEventListener("change", function (event) {
      ...
      if (file) {
        ...
        console.warn(
          "enviarImg.js: Evento 'change' disparado, mas nenhum arquivo encontrado.",
        ); // AVISO B
      }
    });
  } else {
    console.error(
      "enviarImg.js: ERRO - Elemento .questaoImg não encontrado para adicionar listener de 'change'!",
    ); // ERRO C
  }
});
...
async function enviarImgParaAnalise(file) {
  ...
  if (!pergunta || !resposta || !respostaIA) {
    console.error(
      "enviarImg.js: ERRO - Textarea 'pergunta', 'respostaAluno' ou 'result' não encontrado!",
    ); // ERRO D
    return;
  }
  ...
  try {
    const response = await fetch(
      "https://backend-simpatia.onrender.com/analisar-imagem",
      {
        method: "POST",
        body: formData,
      },
    ...
    if (typeof mostrarNotificacao === "function") {
      mostrarNotificacao(
        'Texto extraído! Revise e clique em "Gerar Correção".',
      );
    }
  } catch (error) {
